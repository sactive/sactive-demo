...
class Logger {
  constructor($$utils) {
    let dailyFileName = `${$$utils.envs('SBOT_LOG_DIR')}/sbot-servive-controller-%DATE%.log`;
    let sbotFormat = printf(info => {
      info.stack = info.stack ? '\n' + info.stack : '';
      return `${info.timestamp} [${info.label}] ${info.level}: ${info.message}${info.stack}`;
    });
    let sbotFormatLabel = {label: $$utils.envs('SBOT_LOG_LABEL')};
    ...
    let dailyRotateFileTransport = new (logger.transports.DailyRotateFile)({
      ...
      maxSize: '20m',
      maxFiles: $$utils.envs('SBOT_MAX_LOG_FILES_TIME'),
